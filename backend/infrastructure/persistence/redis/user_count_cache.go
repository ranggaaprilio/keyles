...
// UserCountCache provides caching for tenant user counts. Mirrors the
// pattern established by ClientCountCache.
...
func (c *UserCountCache) Get(ctx context.Context, tenantID string) (int, bool, error) {
	key := c.key(tenantID)
	...
	if err != nil {
		...
		return 0, false, fmt.Errorf("failed to get user count from cache: %w", err)
	}

	count, err := strconv.Atoi(val)
	if err != nil {
		return 0, false, fmt.Errorf("invalid cached count value: %w", err)
	}
