...
CREATE TABLE IF NOT EXISTS user_events (
    ...
    WITH
        ...
        CONSTRAINT user_events_valid_type CHECK (
            event_type IN (
                'login_success',
                'login_failure',
                'token_refresh',
...
CREATE INDEX idx_user_events_user_recent ON user_events (user_id, occurred_at DESC);

-- Admin filtering by tenant + event type
CREATE INDEX idx_user_events_tenant_type ON user_events (tenant_id, event_type, occurred_at DESC);
