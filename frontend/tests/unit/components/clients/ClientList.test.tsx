...
describe('ClientList', () => {
  ...
  it('renders client cards from data', () => {
    render(<ClientList onSelectClient={mockOnSelectClient} onCreateNew={mockOnCreateNew} />);
  ...
  it('shows empty state when no clients', () => {
    ...
    render(<ClientList onSelectClient={mockOnSelectClient} onCreateNew={mockOnCreateNew} />);
    expect(screen.getByText(/no client applications registered/i)).toBeInTheDocument();
  });
  ...
  it('renders pagination controls for multi-page results', () => {
    mockUseClients.mockReturnValue({
      data: { clients: mockClients, total: 25, page: 1, page_size: 10, total_pages: 3 },
      isLoading: false,
      isError: false,
    });
    render(<ClientList onSelectClient={mockOnSelectClient} onCreateNew={mockOnCreateNew} />);
