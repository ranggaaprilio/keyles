...
export function IntegrationDocs({ client }: IntegrationDocsProps) {
  ...
  const redirectUri =
    client.redirect_uris[0] || "https://your-app.com/callback";
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Tabs
          defaultValue={isConfidential ? "auth-code" : "pkce"}
          className="w-full"
        >
  ...
  -d "client_id=${client.client_id}"${isConfidential ? ' \\\n  -d "client_secret=YOUR_CLIENT_SECRET"' : ""}`}
        ...
        <div className="mt-4 pt-4 border-t">
          <p className="text-xs text-muted-foreground">
            ...
            development. Fragment identifiers (#) are not allowed per the OAuth
            2.0 specification.
          </p>
        </div>
      </CardContent>
