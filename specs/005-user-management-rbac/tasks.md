...
- [x] T029 [US7] Modify `backend/usecase/auth/authorize_client.go`: inject `RoleRepository repositories.RoleRepository` as a new dependency in `AuthorizeClient` struct; before generating the authorization code, call `roleRepo.HasAnyRole(ctx, req.UserID, req.ClientID)` — if false, return `&OAuthError{Code: ErrAccessDenied, Description: "user has no active role for this application"}` (FR-021); update `NewAuthorizeClient` constructor accordingly
